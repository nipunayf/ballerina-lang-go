...
#
# + x - float value to operate on
# + return - largest float value not greater than parameter `x` that is a mathematical integer
public isolated function floor(float x) returns float = external;

# Rounds a float up to the closest integral value.
#
# + x - float value to operate on
# + return - smallest float value not less than parameter `x` that is a mathematical integer
...
# Raises one float value to the power of another float value.
#
...
#
# + x - float value to operate on
# + return - Euler's number raised to the power of parameter `x`
...
#
# Corresponds to IEEE atan2(y, x) operation.
#
# + y - the y-coordinate
# + x - the x-coordinate
...
# This is the inverse of function ``value:toString`` applied to a `float`.
#
...
# Returns NaN if there are no arguments.
#
# + xs - float values
