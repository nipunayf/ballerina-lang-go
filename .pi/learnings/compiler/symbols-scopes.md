...
- **No `SymbolLocation` method** on CompilerContext — location is on `model.Symbol` interface but not exposed via CompilerContext. `explore-codebase/context/context.go`
  - The old `ls-ref` completion used `cx.SymbolLocation(ref)` which does NOT exist on the current `CompilerContext`. The new design avoids this by using `CompletionIndex` (module-level facts) + red-node tree (parameters/locals).
- **No `SymbolIsClass` method** on CompilerContext — not present in actual code.
- **No `SymbolPackage` method** on CompilerContext — not present in actual code.
- **No `ValueSymbolMetadata` method** on CompilerContext — not present in actual code.
- **No `DistinctTypeID`/`DistinctTypeSymbolRef` methods** on CompilerContext — not present in actual code.
- **No `SymbolAnnotationValues` method** on CompilerContext — not present in actual code.
...
- `model.SymbolSpace.Symbols()` — returns `iter.Seq2[int, Symbol]` for iteration — `explore-codebase/model/symbol.go:180-190`
...
- `model.SymbolSpace` — thread-safe (`mu sync.RWMutex` on lookup table + symbols slice); `GetSymbol(name)`, `SymbolAt(index)`, `AppendSymbol(sym)`, `RefAt(index)`, `Symbols() iter.Seq2`, `Len()`, `SpaceIndex()`, `Pkg` — `explore-codebase/model/symbol.go:180-190`
