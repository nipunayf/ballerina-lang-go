...
- [tickets.md](tickets.md) — backlog dependency chain and per-ticket findings (status, Java oracle contexts, decisions) for tickets 18, 20/21, 22, 36; ticket 15 and 17 are noted only as the open frontier, with no dedicated section.
- [protocol-free-dtos.md](protocol-free-dtos.md) — Core-service seam (protocol-free core, server-only LSP adaptation), CompletionItemKind ownership (3-layer kind chain: projects → query → server), dependency direction invariants.
